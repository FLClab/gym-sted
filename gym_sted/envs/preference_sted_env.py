...
class PreferenceCountRateScaleRewardEngSTEDMultiObjectivesEnv(PreferenceCountRateScaleSTEDMultiObjectivesEnv):
    ...
    def __init__(self, bleach_sampling="constant", actions=["p_sted"],
        ...
        super(PreferenceCountRateScaleRewardEngSTEDMultiObjectivesEnv, self).__init__(
            bleach_sampling = bleach_sampling,
            actions = actions,
            max_episode_steps = max_episode_steps,
            scale_nanodomain_reward = scale_nanodomain_reward,
    ...
    def step(self, action):
        ...
        # Reward is scaled based on the action that is taken.
        # The agent should seek to minimize the light dose at the sample
        norm_action = self.action_normalizer(action)
        reward -= norm_action.sum() / len(norm_action)
