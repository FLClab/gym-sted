...
class timedExpSTEDEnv2SingleRwrd(gym.Env):
    """
    Creates a 'STEDEnv'
    ...
    ***
    This is the same as timedExpSTEDEnv2 except that there is only 1 reward at the end of each episode instead of
    returning a reward everytime an acquisition with a p_sted > threshold is made ...
    how de fok do I give only 1 reward per episode and make sure the agent knows which picture taken at which time
    with which params gave the reward?
    ***
    """
    metadata = {'render.modes': ['human']}
    obj_names = ["Resolution", "Bleach", "SNR", "NbNanodomains"]
...
class timedExpSTEDEnv2Bump(gym.Env):
    ...
    ***
    This is the same exp env as timedExpSTEDEnv2, except the light curve has a little warning step before the flash
    is filled, and the flashes update more frequently
    ***
    """
    metadata = {'render.modes': ['human']}
    obj_names = ["Resolution", "Bleach", "SNR", "NbNanodomains"]
